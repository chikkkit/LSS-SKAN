...
# 单参数非线性函数(Single-parameterized function)
lfuns = ['lrelu', 'lleaky_relu', 'lswish', 'lmish', 'lsoftplus', 'lhard_sigmoid', 
         'lelu', 'lshifted_softplus', 'lge lup']

# 测试这些单变量函数的效果，并将结果保存在dataframe中(Test the effect of these univariate functions and save the results in the dataframe)
res = None

# 计算网络的参数量(Calculate the number of parameters in the network)
...
for lr in [*np.linspace(0.01, 0.09, 9), *np.linspace(0.001, 0.009, 9), *np.linspace(0.0001, 0.0009, 10)]:
    for lfun in lfuns:
        ...
        for epoch in tqdm.trange(10):
            net.train()
            # 记录训练的时间(Record the training time)
            ...
            for x, y in train_loader:
                ...
                optimizer.step()
            end_time = time.time()
            # 计算训练集上的准确率和损失(Calculate the accuracy and loss on the training set)
            net.eval()
            correct = 0
            total = 0
            loss = 0
            ...
            # 计算测试集上的准确率和损失(Calculate the accuracy and loss on the test set)
            net.eval()
            correct = 0
            total = 0
            loss = 0
